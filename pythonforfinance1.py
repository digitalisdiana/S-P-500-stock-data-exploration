...
style.use("ggplot") #Use this specific plot
...
df["90ma"] = df["Adj Close"].rolling(window = 90, min_periods = 0).mean() #Feel free to copy and test it
...
df_ohlc = df["Adj Close"].resample("45min").ohlc() #Defining OHLC data with resampled data points for every 45 minutes
df_volume = df["Volume"].resample("45min").sum() #Resampling the volume just the same but asking for a ".sum()" as to reflect the actual {true} volume
...
df.ohlc["Date"] = df.ohlc["Date"].map(mdates.date2num) #Map out the dates after converting our current date time objects to matplotlib's mdates
